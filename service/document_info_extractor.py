...
class DocumentInformationExtractor:
    ...
    def extract_document_info_sections(self, act_url):
        """
        Extract the sections of the document information html page, in particular different sections are divided
        into div blocks with specific id: Dates, MiscellaneousInformation, Procedure, ...
         """
        try:
            html_metadata = requests.get(act_url).text

            soup = BeautifulSoup(html_metadata, 'html.parser')
            section = soup.find_all('div', id=lambda x: x and x.startswith('PPLinked_Contents'))
            relationships = self.extract_relationship_between_documents_section(section)
            print(relationships)
